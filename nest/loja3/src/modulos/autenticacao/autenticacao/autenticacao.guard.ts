...
export class AutenticacaoGuard implements CanActivate {
  ...
  async canActivate(contexto: ExecutionContext): Promise<boolean> {
    const requisicao = contexto
      .switchToHttp()
      .getRequest<RequisicaoComUsuario>(); /* dá um controle maior do tipo de dado q vai receber */
    const token = this.extrairTokenDoCabecalho(requisicao);

    if (!token) {
      throw new UnauthorizedException('Erro de autentificação');
    }
  ...
  private extrairTokenDoCabecalho(requisicao: Request): string | undefined {
    ...
    return tipo === 'Bearer' ? token : undefined; /* ternário */
  }
}
