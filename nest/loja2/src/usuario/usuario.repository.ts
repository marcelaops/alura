...
export class UsuarioRepository {
  ...
  async existeComEmail(email: string) {
    ...
    //vamos comparar se existe esse email, caso exista vai retornar algo. Caso não exista, vai retornar undefined.
  ...
  async atualiza(id: string, dadosDeAtualizacao: Partial<UsuarioEntity>) {
    ...
    Object.entries(dadosDeAtualizacao).forEach(([chave, valor]) => {
      if(chave === 'id') return;
      usuario[chave] = valor; /* atualização de fato dos dados de atualização */
    })
